...
const simulation = {
	...
	then: null,
	introPlayer() {
		//not work, but trying to show player in the intro screen
		setTimeout(() => {
	...
	checks() {
		if (!(m.cycle % 60)) {
			...
			if (m.pos.y > simulation.fallHeight || isNaN(player.position.x)) {
				// if 4000px deep
	...
	enableConstructMode() {
		...
		document.body.addEventListener('mouseup', e => {
			if (simulation.testing && simulation.constructMouseDownPosition) {
				...
				} else if (simulation.mouseInGame.x > simulation.constructMouseDownPosition.x && simulation.mouseInGame.y > simulation.constructMouseDownPosition.y) {
		...
		document.body.addEventListener('keydown', e => {
			...
			if (simulation.testing && e.keyCode === 90 && simulation.constructMapString.length) {
				if (simulation.constructMapString[simulation.constructMapString.length - 1][6] === 'm') {
					//remove map from current level
					const index = map.length - 1
					Matter.Composite.remove(engine.world, map[index])
					map.splice(index, 1)
					simulation.draw.setPaths() //update map graphics
				} else if (simulation.constructMapString[simulation.constructMapString.length - 1][6] === 'b') {
	...
	outputMapString(string) {
		...
		navigator.clipboard.writeText(out).then(
			...
			function () {
				/* clipboard write failed */ console.log('copy failed')
			}
		)
