	...
	spawn(xPos, yPos, sides, radius, color) {
		...
		mob[i] = Matter.Bodies.polygon(xPos, yPos, sides, radius, {
			...
			searchSpring() {
				...
				if (!(simulation.cycle % this.seePlayerFreq)) {
					if ((this.seePlayer.recall || this.isLookingAtPlayer(this.lookRange)) && this.distanceToPlayer2() < this.seeAtDistance2 && Matter.Query.ray(map, this.position, player.position).length === 0 && Matter.Query.ray(body, this.position, player.position).length === 0 && !m.isCloak) {
						this.foundPlayer()
					} else if (this.seePlayer.recall) {
						this.lostPlayer()
					}
