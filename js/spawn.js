...
const spawn = {
	...
	randomBossList: ['shieldingBoss', 'orbitalBoss', 'historyBoss', 'shooterBoss', 'cellBossCulture', 'bomberBoss', 'spiderBoss', 'launcherBoss', 'laserTargetingBoss', 'powerUpBoss', 'powerUpBossBaby', 'snakeBoss', 'streamBoss', 'pulsarBoss', 'spawnerBossCulture', 'grenadierBoss', 'growBossCulture', 'blinkBoss', 'snakeSpitBoss', 'laserBombingBoss', 'blockBoss', 'revolutionBoss', 'slashBoss'],
	...
	fullPickList: ['hopper', 'hopper', 'hopper', 'slasher', 'slasher', 'hopper', 'stabber', 'stabber', 'stabber', 'springer', 'springer', 'springer', 'shooter', 'shooter', 'grenadier', 'grenadier', 'striker', 'striker', 'laser', 'laser', 'pulsar', 'pulsar', 'launcher', 'launcherOne', 'exploder', 'sneaker', 'sucker', 'sniper', 'spinner', 'grower', 'beamer', 'focuser', 'spawner', 'ghoster'],
	...
	randomSmallMob(x, y, num = Math.max(Math.min(Math.round(Math.random() * simulation.difficulty * 0.2), 4), 0), size = 16 + Math.ceil(Math.random() * 15), chance = 1) {
	...
	WIMP(x = level.exit.x + 300 * (Math.random() - 0.5), y = level.exit.y + 300 * (Math.random() - 0.5)) {
		...
		me.do = function () {
			//wake up 2 seconds after the player moves
	...
	finalBoss(x, y, radius = 300) {
		...
		me.modeLasers = function () {
			...
			if (this.cycle < 240) {
				//damage scales up over 2 seconds to give player time to move
		...
		me.do = function () {
			...
			if (!(simulation.cycle % 30)) {
				...
				for (let i = 0; i < body.length; i++) {
					if (Vector.magnitude(Vector.sub(this.position, body[i].position)) < 700 && !body[i].isNotHoldable) {
	...
	hopBoss(x, y, radius = 90) {
		...
		me.do = function () {
			...
			if (this.seePlayer.recall) {
				...
				if (deltaSpeed > 13 && this.speed < 5) {
					//if the player slows down greatly in one cycle
				...
				if (this.isInAir) {
					if ((this.velocity.y > -0.01 && Matter.Query.collides(this, map).length) || Matter.Query.collides(this, body).length) {
						//not moving up, and has hit the map or a body
	...
	sprayBoss(x, y, radius = 30, isSpawnBossPowerUp = true) {
		...
		me.noFire = function () {
			this.normalDoStuff()
			this.phaseCycle++
			if (this.phaseCycle > this.noFireTotalCycles) {
				//start burst fire mode
				this.phaseCycle = -2
	...
	slashBoss(x, y, radius = 80) {
		...
		me.swordWaiting = function () {
			if (this.seePlayer.recall && this.cd < simulation.cycle && this.distanceToPlayer2() < seeDistance2 && Matter.Query.ray(map, this.position, this.playerPosRandomY()).length === 0 && Matter.Query.ray(body, this.position, this.playerPosRandomY()).length === 0) {
	...
	boost(x, y, height = 1000) {
		...
		level.addQueryRegion(x, y - 20, 100, 20, 'boost', [[player], body, mob, powerUp, bullet], -1.21 * Math.sqrt(Math.abs(height)))
	},
