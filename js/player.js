...
const m = {
	...
	switchWorlds() {
		...
		for (let i = 0, len = tech.tech.length; i < len; i++) {
			...
			if (tech.tech[i].count > 0 && !tech.tech[i].isLore) {
				...
				if (!tech.tech[i].isNonRefundable && !tech.tech[i].isFromAppliedScience && tech.tech[i].name !== 'many-worlds' && tech.tech[i].name !== 'Ψ(t) collapse' && tech.tech[i].name !== 'non-unitary operator' && tech.tech[i].name !== '-quantum leap-') {
	...
	drawFieldMeterCloaking: function () {
		if (m.energy < m.maxEnergy) {
			// replaces m.drawFieldMeter() with custom code
			m.regenEnergy()
			const xOff = m.pos.x - m.radius * m.maxEnergy
			const yOff = m.pos.y - 50
	...
	regenEnergy: function () {
		//used in drawFieldMeter  // rewritten by some tech
	...
	grabPowerUp() {
		...
		for (let i = 0, len = powerUp.length; i < len; ++i) {
			...
			if (dist2 < m.grabPowerUpRange2 && (m.lookingAt(powerUp[i]) || dist2 < 16000) && Matter.Query.ray(map, powerUp[i].position, m.pos).length === 0) {
	...
	pushMass(who, fieldBlockCost = (0.025 + Math.sqrt(who.mass) * Vector.magnitude(Vector.sub(who.velocity, player.velocity)) * 0.002) * m.fieldShieldingScale) {
		if (m.energy > fieldBlockCost * 0.2) {
	...
	pushMobsFacing() {
		...
		for (let i = 0, len = mob.length; i < len; ++i) {
			if (Vector.magnitude(Vector.sub(mob[i].position, m.pos)) - mob[i].radius < m.fieldRange && m.lookingAt(mob[i]) && !mob[i].isUnblockable && Matter.Query.ray(map, mob[i].position, m.pos).length === 0) {
				mob[i].locatePlayer()
				m.pushMass(mob[i])
		...
		},
		{
			...
			effect: () => {
				...
				m.harmonic3Phase = () => {
					...
					//360 block
					for (let i = 0, len = mob.length; i < len; ++i) {
						if (Vector.magnitude(Vector.sub(mob[i].position, m.pos)) - mob[i].radius < netfieldRange && !mob[i].isUnblockable) {
					...
					if (tech.isPerfectBrake) {
						...
						for (let i = 0; i < mob.length; i++) {
							...
							if (distance < range) {
								...
								if (mob[i].speed > cap && Vector.dot(mob[i].velocity, Vector.sub(m.pos, mob[i].position)) > 0) {
						...
						if (m.isHolding) {
							...
							if (!m.holdingTarget) {
								...
								if (this.rewindCount > 599 || m.energy < DRAIN) {
									...
									//grab power ups
									for (let i = 0, len = powerUp.length; i < len; ++i) {
										if (Vector.magnitudeSquared(Vector.sub(m.pos, powerUp[i].position)) < 100000 && !simulation.isChoosing && (powerUp[i].name !== 'heal' || m.health !== m.maxHealth || tech.isOverHeal)) {
											powerUps.onPickUp(powerUp[i])
											powerUp[i].effect()
											Matter.Composite.remove(engine.world, powerUp[i])
											powerUp.splice(i, 1)
					...
					if (input.field) {
						if (m.fieldCDcycle < m.cycle) {
							...
							for (let i = 0, len = powerUp.length; i < len; ++i) {
								...
								if (dist2 < m.fieldRadius * m.fieldRadius && (m.lookingAt(powerUp[i]) || dist2 < 16000)) {
									powerUp[i].force.x += 0.05 * (dxP / Math.sqrt(dist2)) * powerUp[i].mass
									powerUp[i].force.y += 0.05 * (dyP / Math.sqrt(dist2)) * powerUp[i].mass - powerUp[i].mass * simulation.g //negate gravity
									//extra friction
									Matter.Body.setVelocity(powerUp[i], {
							...
							if (m.energy > 0.01) {
								...
								if (Matter.Query.ray(map, m.fieldPosition, player.position).length) {
						...
						if (tech.isWormBullets) {
							...
							for (let i = 0, len = bullet.length; i < len; ++i) {
								...
								if (!bullet[i].botType && !bullet[i].isInHole) {
									//don't teleport bots
									if (Vector.magnitude(Vector.sub(m.hole.pos1, bullet[i].position)) < m.fieldRange) {
									...
									} else if (Vector.magnitude(Vector.sub(m.hole.pos2, bullet[i].position)) < m.fieldRange) {
										//find if bullet is touching hole1
