const tech = {
	...
	//     }
	// },
	addJunkTechToPool(chance) {
		//chance is number between 0-1
		// { //count JUNK
		//     let count = 0
		//     for (let i = 0, len = tech.tech.length; i < len; i++) {
	...
	damageFromTech() {
		...
		if (tech.isEnergyNoAmmo) dmg *= 1.7
	...
	tech: [
		...
		},
		{
			...
			junk: 0.05,
			descriptionFunction() {
				return `double your current <strong class='color-ammo'>ammo</strong> for all <strong class='color-g'>guns</strong>`
			},
			maxCount: 9,
			count: 0,
			frequency: 1,
		...
		// },
		{
			...
			frequencyDefault: 1,
			allowed() {
				//&& (m.fieldUpgrades[m.fieldMode].name !== "molecular assembler" || m.maxEnergy > 1)
		...
		},
		{
			...
			effect: () => {
				...
				for (let i = 0; i < 2; i++) {
					//double chance for dynamo-bot, since it's very good for assembler
					notUpgradedBots.push(() => {
		...
		},
		{
			...
			effect() {
				...
				for (let i = 0, len = tech.tech.length; i < len; i++) {
					if (tech.tech[i].count < tech.tech[i].maxCount && tech.tech[i].allowed() && !tech.tech[i].isJunk && !tech.tech.isLore) {
						options.push(i)
					}
				}
				if (options.length) {
		...
		},
		{
			...
			effect() {
				m.draw = function () {
					...
					ctx.beginPath()
					ctx.moveTo(-30, 0)
					ctx.bezierCurveTo(-65, -75, -5, 150 + 5 * Math.sin(simulation.cycle / 10), -70 + 10 * Math.sin(simulation.cycle / 10), 0 + 10 * Math.sin(simulation.cycle / 10))
					ctx.strokeStyle = '#333'
					ctx.lineWidth = 4
					ctx.stroke()
