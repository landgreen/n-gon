...
const b = {
	...
	pulse(charge, angle = m.angle, where = m.pos) {
		...
		if (tech.isPulseAim) {
			//find mobs in line of sight
			let dist = 2200
			for (let i = 0, len = mob.length; i < len; i++) {
				const newDist = Vector.magnitude(Vector.sub(path[0], mob[i].position))
				if (explosionRadius < newDist && newDist < dist && !mob[i].isBadTarget && Matter.Query.ray(map, path[0], mob[i].position).length === 0 && Matter.Query.ray(body, path[0], mob[i].position).length === 0) {
					dist = newDist
					best.who = mob[i]
	...
	setGrenadeMode() {
		...
		grenadeRPGVacuum = function (where = { x: m.pos.x + 30 * Math.cos(m.angle), y: m.pos.y + 30 * Math.sin(m.angle) }, angle = m.angle, size = 1) {
			...
			bullet[me].do = function () {
				const suckCycles = 40
				if (simulation.cycle > this.endCycle - suckCycles || Matter.Query.collides(this, map).length || Matter.Query.collides(this, body).length) {
					//suck
					const that = this
	...
	dart(where, angle = m.angle, size = 0.8) {
		...
		for (let i = 0, len = mob.length; i < len; ++i) {
			if (mob[i].alive && !mob[i].isBadTarget && Matter.Query.ray(map, where, mob[i].position).length === 0) {
				...
				if (dist < closest.score && dot > 0.9 - 0.0004 * dist) {
		...
		bullet[me] = Bodies.fromVertices(
			...
			where.y,
			[
				{ x: -20 * size, y: 2 * size, index: 0, isInternal: false },
				{ x: -20 * size, y: -2 * size, index: 1, isInternal: false },
				{ x: 5 * size, y: -2 * size, index: 4, isInternal: false },
				{ x: 20 * size, y: 0, index: 3, isInternal: false },
				{ x: 5 * size, y: 2 * size, index: 4, isInternal: false },
			],
			{
				cycle: 0,
	...
	grapple(where, angle = m.angle, harpoonSize = 1) {
		...
		bullet[me] = Bodies.fromVertices(
			...
			where.y,
			[
				{ x: -50 * harpoonSize, y: 2 * harpoonSize, index: 0, isInternal: false },
				{ x: -50 * harpoonSize, y: -2 * harpoonSize, index: 1, isInternal: false },
				{ x: 45 * harpoonSize, y: -3 * harpoonSize, index: 2, isInternal: false },
				{ x: 50 * harpoonSize, y: 0, index: 3, isInternal: false },
				{ x: 45 * harpoonSize, y: 3 * harpoonSize, index: 4, isInternal: false },
			],
			{
				angle: angle,
				friction: 1,
				...
				returnToPlayer() {
					if (Vector.magnitude(Vector.sub(this.position, m.pos)) < returnRadius) {
						//near player
						this.endCycle = 0
				...
				do() {
					if (input.fire) {
						...
						if (this.endCycle < simulation.cycle + 1) {
							//if at end of lifespan, but player is holding down fire, force retraction
	...
	harpoon(where, target, angle = m.angle, harpoonSize = 1, isReturn = false, totalCycles = 35) {
		...
		bullet[me] = Bodies.fromVertices(
			...
			where.y,
			[
				{ x: -40 * harpoonSize, y: 2 * harpoonSize, index: 0, isInternal: false },
				{ x: -40 * harpoonSize, y: -2 * harpoonSize, index: 1, isInternal: false },
				{ x: 50 * harpoonSize, y: -3 * harpoonSize, index: 3, isInternal: false },
				{ x: 30 * harpoonSize, y: 2 * harpoonSize, index: 4, isInternal: false },
			],
			{
				cycle: 0,
				angle: angle,
	...
	missile(where, angle, speed, size = 1) {
		...
		bullet[me] = Bodies.rectangle(where.x, where.y, 30 * size, 4 * size, {
			...
			do() {
				...
				ctx.arc(this.position.x - Math.cos(this.angle) * (25 * size - 3) + (Math.random() - 0.5) * 4, this.position.y - Math.sin(this.angle) * (25 * size - 3) + (Math.random() - 0.5) * 4, 11 * size, 0, 2 * Math.PI)
		...
		bullet[me] = Bodies.polygon(position.x, position.y, 3, 25, {
			...
			do() {
				if (!(simulation.cycle % this.lookFrequency) && m.energy > this.drain) {
					//find mob targets
					for (let i = 0, len = mob.length; i < len; ++i) {
						if (Vector.magnitude(Vector.sub(this.position, mob[i].position)) < 1300 && !mob[i].isBadTarget && Matter.Query.ray(map, this.position, mob[i].position).length === 0 && Matter.Query.ray(body, this.position, mob[i].position).length === 0) {
					...
					if (simulation.cycle > this.lookFrequency) {
						...
						this.do = function () {
							...
							if (!(simulation.cycle % this.lookFrequency)) {
								...
								for (let i = 0, len = mob.length; i < len; ++i) {
									if (!mob[i].isBadTarget && Vector.magnitude(Vector.sub(this.position, mob[i].position)) < 700 + mob[i].radius + random && Matter.Query.ray(map, this.position, mob[i].position).length === 0 && Matter.Query.ray(body, this.position, mob[i].position).length === 0) {
		...
		bullet[me] = Bodies.polygon(where.x, where.y, 3, RADIUS, {
			...
			do() {
				...
				if (!(simulation.cycle % this.lookFrequency)) {
					...
					let closeDist = Infinity
					for (let i = 0, len = mob.length; i < len; ++i) {
						if (!mob[i].isBadTarget && Matter.Query.ray(map, this.position, mob[i].position).length === 0 && Matter.Query.ray(body, this.position, mob[i].position).length === 0) {
							...
							if (this.lockedOn) {
								...
								for (let i = 0, len = powerUp.length; i < len; ++i) {
									if (
										...
										//look for power ups to lock onto
										if (Matter.Query.ray(map, this.position, powerUp[i].position).length === 0 && Matter.Query.ray(body, this.position, powerUp[i].position).length === 0) {
		...
		bullet[me] = Bodies.polygon(position.x, position.y, 20, radius, {
			...
			do() {
				...
				if (this.target && this.target.alive) {
					...
					//slow when touching map or blocks
					const slow = 0.85
					Matter.Body.setVelocity(this, {
						x: this.velocity.x * slow,
						y: this.velocity.y * slow,
	...
	randomBot(where = player.position, isKeep = true, isLaser = true) {
		if (Math.random() < 0.5) {
			//chance to match scrap bot to your upgrade
			...
			//else don't match scrap bot to upgrade
		...
		bullet[me] = Bodies.polygon(position.x, position.y, 4, RADIUS, {
			...
			do() {
				...
				if (distanceToPlayer > this.range) {
					...
					//close to player
					Matter.Body.setVelocity(this, Vector.add(Vector.mult(this.velocity, 0.9), Vector.mult(player.velocity, 0.17))) //add player's velocity
					if (this.lastLookCycle < simulation.cycle && !m.isCloak) {
						this.lastLookCycle = simulation.cycle + this.delay
						for (let i = 0, len = mob.length; i < len; i++) {
							const dist = Vector.magnitudeSquared(Vector.sub(this.position, mob[i].position))
							if (!mob[i].isBadTarget && dist < 3000000 && Matter.Query.ray(map, this.position, mob[i].position).length === 0 && Matter.Query.ray(body, this.position, mob[i].position).length === 0 && !mob[i].isShielded) {
				...
				if (distanceToPlayer > this.range) {
					...
					if (this.cd < simulation.cycle && !(simulation.cycle % this.lookFrequency) && !m.isCloak) {
						for (let i = 0, len = mob.length; i < len; i++) {
							...
							if (mob[i].alive && !mob[i].isBadTarget && dist2 > 40000 && Matter.Query.ray(map, this.position, mob[i].position).length === 0) {
				...
				if (!(simulation.cycle % this.lookFrequency)) {
					...
					if (!m.isCloak) {
						...
						for (let i = 0, len = mob.length; i < len; ++i) {
							...
							if (DIST - mob[i].radius < closeDist && !mob[i].isShielded && (!mob[i].isBadTarget || mob[i].isMobBullet) && Matter.Query.ray(map, this.vertices[0], mob[i].position).length === 0 && Matter.Query.ray(body, this.vertices[0], mob[i].position).length === 0) {
								closeDist = DIST
								this.lockedOn = mob[i]
							}
						}
		...
		bullet[me] = Bodies.polygon(position.x, position.y, 4, RADIUS, {
			...
			do() {
				...
				if (distanceToPlayer > 100) {
					...
					if (!(simulation.cycle % this.lookFrequency) && !m.isCloak) {
						...
						for (let i = 0, len = mob.length; i < len; ++i) {
							...
							if (DIST < closeDist && !mob[i].isBadTarget && Matter.Query.ray(map, this.position, mob[i].position).length === 0 && Matter.Query.ray(body, this.position, mob[i].position).length === 0) {
								closeDist = DIST
								this.lockedOn = mob[i]
							}
						}
				...
				if (this.lockedOn && this.lockedOn.alive && !m.isCloak) {
					...
					if (DIST - this.lockedOn.radius < this.range && Matter.Query.ray(map, this.position, this.lockedOn.position).length === 0) {
			...
			do() {
				...
				if (!(simulation.cycle % this.lookFrequency)) {
					...
					if (!m.isCloak) {
						...
						for (let i = 0, len = mob.length; i < len; ++i) {
							const DIST = Vector.magnitude(Vector.sub(this.position, mob[i].position)) - mob[i].radius
							if (DIST < closeDist && (!mob[i].isBadTarget || mob[i].isMobBullet) && Matter.Query.ray(map, this.position, mob[i].position).length === 0 && Matter.Query.ray(body, this.position, mob[i].position).length === 0) {
								closeDist = DIST
								this.lockedOn = mob[i]
							}
						}
				...
				if (this.charge > 0) {
					...
					if (!input.fire && this.charge > 0.6) {
						...
						function magField(mag, arc) {
							ctx.moveTo(X, Y)
							ctx.bezierCurveTo(X + unitVector.x * mag, Y + unitVector.y * mag, X + unitVector.x * mag + unitVectorPerp.x * arc, Y + unitVector.y * mag + unitVectorPerp.y * arc, X + unitVectorPerp.x * arc, Y + unitVectorPerp.y * arc)
							ctx.bezierCurveTo(X - unitVector.x * mag + unitVectorPerp.x * arc, Y - unitVector.y * mag + unitVectorPerp.y * arc, X - unitVector.x * mag, Y - unitVector.y * mag, X, Y)
						}
