...
const powerUps = {
  ...
  heal: {
    ...
    effect() {
      let heal = 0;
      for (let i = 0; i < b.modRecursiveHealing; i++)
        heal += (this.size / 40) ** 2;
      if (heal > 0)
        game.makeTextLog(
          "<div class='circle heal'></div> &nbsp; <span style='font-size:115%;'> <strong style = 'letter-spacing: 2px;'>heal</strong>  " +
            (
              Math.min(mech.maxHealth - mech.health, heal) *
              game.healScale *
              100
            ).toFixed(0) +
            "%</span>",
  ...
  ammo: {
    ...
    effect() {
      ...
      if (b.inventory.length > 0) {
        ...
        target =
          b.guns[b.inventory[Math.floor(Math.random() * b.inventory.length)]];
        ...
        if (target.ammo === Infinity) {
          ...
          if (target.ammo === Infinity) {
            target =
              b.guns[
                b.inventory[Math.floor(Math.random() * b.inventory.length)]
              ];
            if (target.ammo === Infinity)
              target =
                b.guns[
        ...
        if (target.ammo === Infinity) {
          ...
          if (!game.lastLogTime)
            game.makeTextLog(
              "<span style='font-size:115%;'><span class='color-f'>+energy</span></span>",
              300
            );
        } else {
          ...
          game.updateGunHUD();
          game.makeTextLog(
            "<div class='circle gun'></div> &nbsp; <span style='font-size:110%;'>+" +
              ammo +
              " ammo for " +
              target.name +
              "</span>",
    ...
    effect() {
      function pick(who, skip1 = -1, skip2 = -1, skip3 = -1) {
        ...
        for (let i = 1; i < who.length; i++) {
          if (
            i !== mech.fieldMode &&
            (!game.isEasyToAimMode || mech.fieldUpgrades[i].isEasyToAim) &&
            i !== skip1 &&
            i !== skip2 &&
            i !== skip3
          )
      ...
      if (choice1 > -1) {
        ...
        if (choice2 > -1)
          text += `<div class="choose-grid-module" onclick="powerUps.choose('field',${choice2})"><div class="grid-title"><div class="circle-grid field"></div> &nbsp; ${mech.fieldUpgrades[choice2].name}</div> ${mech.fieldUpgrades[choice2].description}</div>`;
        if (!b.isModBayesian) {
          choice3 = pick(mech.fieldUpgrades, choice1, choice2);
          if (choice3 > -1)
            text += `<div class="choose-grid-module" onclick="powerUps.choose('field',${choice3})"><div class="grid-title"><div class="circle-grid field"></div> &nbsp; ${mech.fieldUpgrades[choice3].name}</div> ${mech.fieldUpgrades[choice3].description}</div>`;
        }
        if (b.isModFourOptions) {
          let choice4 = pick(mech.fieldUpgrades, choice1, choice2, choice3);
          if (choice4 > -1)
            text += `<div class="choose-grid-module" onclick="powerUps.choose('field',${choice4})"><div class="grid-title"><div class="circle-grid field"></div> &nbsp; ${mech.fieldUpgrades[choice4].name}</div> ${mech.fieldUpgrades[choice4].description}</div>`;
        }
  ...
  mod: {
    ...
    effect() {
      ...
      if (choice1 > -1) {
        ...
        if (choice2 > -1)
          text += `<div class="choose-grid-module" onclick="powerUps.choose('mod',${choice2})"><div class="grid-title"><div class="circle-grid mod"></div> &nbsp; ${b.mods[choice2].name}</div> ${b.mods[choice2].description}</div>`;
        if (!b.isModBayesian) {
          choice3 = pick(choice1, choice2);
          if (choice3 > -1)
            text += `<div class="choose-grid-module" onclick="powerUps.choose('mod',${choice3})"><div class="grid-title"><div class="circle-grid mod"></div> &nbsp; ${b.mods[choice3].name}</div> ${b.mods[choice3].description}</div>`;
        }
        if (b.isModFourOptions) {
          let choice4 = pick(choice1, choice2, choice3);
          if (choice4 > -1)
            text += `<div class="choose-grid-module" onclick="powerUps.choose('mod',${choice4})"><div class="grid-title"><div class="circle-grid mod"></div> &nbsp; ${b.mods[choice4].name}</div> ${b.mods[choice4].description}</div>`;
        }
        document.getElementById("choose-grid").innerHTML = text;
        powerUps.showDraft();
      } else {
    ...
    effect() {
      function pick(who, skip1 = -1, skip2 = -1, skip3 = -1) {
        ...
        for (let i = 0; i < who.length; i++) {
          if (
            !who[i].have &&
            (!game.isEasyToAimMode || b.guns[i].isEasyToAim) &&
            i !== skip1 &&
            i !== skip2 &&
            i !== skip3
          )
      ...
      if (choice1 > -1) {
        ...
        if (choice2 > -1)
          text += `<div class="choose-grid-module" onclick="powerUps.choose('gun',${choice2})"><div class="grid-title"><div class="circle-grid gun"></div> &nbsp; ${b.guns[choice2].name}</div> ${b.guns[choice2].description}</div>`;
        if (!b.isModBayesian) {
          choice3 = pick(b.guns, choice1, choice2);
          if (choice3 > -1)
            text += `<div class="choose-grid-module" onclick="powerUps.choose('gun',${choice3})"><div class="grid-title"><div class="circle-grid gun"></div> &nbsp; ${b.guns[choice3].name}</div> ${b.guns[choice3].description}</div>`;
        }
        if (b.isModFourOptions) {
          let choice4 = pick(b.guns, choice1, choice2, choice3);
          if (choice4 > -1)
            text += `<div class="choose-grid-module" onclick="powerUps.choose('gun',${choice4})"><div class="grid-title"><div class="circle-grid gun"></div> &nbsp; ${b.guns[choice4].name}</div> ${b.guns[choice4].description}</div>`;
        }
        document.getElementById("choose-grid").innerHTML = text;
        powerUps.showDraft();
      } else {
  ...
  spawnRandomPowerUp(x, y) {
    ...
    if (
      Math.random() * Math.random() - 0.3 > Math.sqrt(mech.health) ||
      Math.random() < 0.035
    ) {
    ...
    if (Math.random() < 0.002 * (3 - b.inventory.length)) {
      //a new gun has a low chance for each not acquired gun up to 3
      powerUps.spawn(x, y, "gun");
      if (Math.random() < b.isModBayesian) powerUps.spawn(x, y, "gun");
      return;
    }
    if (Math.random() < 0.0027 * (14 - b.modCount)) {
      //a new mod has a low chance for each not acquired mod up to 15
  ...
  spawnBossPowerUp(x, y) {
    //boss spawns field and gun mod upgrades
