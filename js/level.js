...
const level = {
	...
	start() {
		if (level.levelsCleared === 0) {
			//this code only runs on the first level
	...
	difficultyDecrease(num = 1) {
		//used in easy mode for simulation.reset()
		for (let i = 0; i < num; i++) {
			simulation.difficulty--
			m.dmgScale /= 0.925 //damage done by player decreases each level
	...
	populateLevels() {
		//run a second time if URL is loaded
		...
		if (simulation.isTraining) {
			...
			if (!build.isExperimentSelection || (build.hasExperimentalMode && !simulation.isCheating)) {
				//experimental mode is endless, unless you only have an experiment Tech
	...
	exit: {
		...
		drawAndCheck() {
			...
			if (level.exitCount > 0) {
				//stroke outline of door from 2 sides,  grows with count
				ctx.beginPath()
	...
	boost(x, y, height = 1000) {
		//height is how high the player will be flung above y
	...
	elevator(x, y, width, height, maxHeight, force = 0.003, friction = { up: 0.01, down: 0.2 }, isAtTop = false) {
		...
		const who = (body[body.length] = Bodies.rectangle(x, isAtTop ? maxHeight : y, width, height, {
			...
			move() {
				if (!m.isBodiesAsleep) {
					if (this.isUp) {
						//moving up still with high air friction
					...
					} else if (this.position.y + 10 * this.velocity.y > y) {
						//free falling down, with only air friction
						Matter.Body.setVelocity(this, {
					...
					if (this.isUp === true) {
						...
						if (list.length > 0) {
							if (list[0].bounds.max.x - list[0].bounds.min.x < 150 && list[0].bounds.max.y - list[0].bounds.min.y < 150) {
								//not too big of a block
								Matter.Body.setPosition(list[0], {
									//teleport block to the center of the button
	...
	vanish(x, y, width, height, isVertical = false, hide = { x: 0, y: 150 }) {
		...
		y = y + height / 2
		const vertices = [
			{ x: x, y: y, index: 0, isInternal: false },
			{ x: x + width, y: y, index: 1, isInternal: false },
			{ x: x + width, y: y + height, index: 4, isInternal: false },
			{ x: x, y: y + height, index: 3, isInternal: false },
		]
		const block = (body[body.length] = Bodies.fromVertices(x, y, vertices, {
			...
			shrinkVertices(size) {
				if (isVertical) {
					return [
						{ x: x, y: y * size, index: 0, isInternal: false },
						{ x: x + width, y: y * size, index: 1, isInternal: false },
						{ x: x + width, y: (y + height) * size, index: 4, isInternal: false },
						{ x: x, y: (y + height) * size, index: 3, isInternal: false },
					]
				} else {
					return [
						{ x: x * size, y: y, index: 0, isInternal: false },
						{ x: (x + width) * size, y: y, index: 1, isInternal: false },
						{ x: (x + width) * size, y: y + height, index: 4, isInternal: false },
						{ x: x * size, y: y + height, index: 3, isInternal: false },
					]
				}
			},
			query() {
				if (this.isThere) {
					...
					} else if (Matter.Query.collides(this, [player]).length) {
						...
						if (this.returnCount < 1) {
							...
							if (Matter.Query.collides(this, [player]).length) {
								...
								for (let i = 0; i < blocks.length; i++) {
									if (blocks[i].bodyB !== this && blocks[i].bodyB !== m.holdingTarget) {
										//dont' delete yourself   <----- bug here maybe...
										Matter.Composite.remove(engine.world, blocks[i].bodyB)
										blocks[i].bodyB.isRemoveMeNow = true
										for (let i = 1; i < body.length; i++) {
											//find which index in body array it is and remove from array
				...
				if (!m.isBodiesAsleep) {
					if (this.isClosing) {
						if (this.position.y < y) {
							//try to close
							if (Matter.Query.collides(this, [player]).length === 0 && Matter.Query.collides(this, body).length < 2 && Matter.Query.collides(this, mob).length === 0) {
								const position = {
									x: this.position.x,
	...
	portal(centerA, angleA, centerB, angleB) {
		...
		query = function (isRemoveBlocks = false) {
			...
			} else if (player.isInPortal !== this) {
				//touching player
				...
				for (let i = 0; i < bullet.length; i++) {
					if (bullet[i].botType) {
						...
						Matter.Body.setPosition(
							bullet[i],
							Vector.add(this.portalPair.portal.position, {
			...
			for (let i = 0, len = body.length; i < len; i++) {
				if (body[i] !== m.holdingTarget) {
					...
					if (Matter.Query.collides(this, [body[i]]).length === 0) {
						...
						//touching this portal, but for the first time
						if (isRemoveBlocks) {
	...
	drip(x, yMin, yMax, period = 100, color = 'hsla(160, 100%, 35%, 0.5)') {
		return {
			...
			draw() {
				if (!m.isBodiesAsleep) {
					if (this.dropCycle < simulation.cycle) {
						//reset
	...
	chain(x, y, angle = 0, isAttached = true, len = 15, radius = 20, stiffness = 1, damping = 1) {
		...
		cons[cons.length] = Constraint.create({
			//pin first block to a point in space
			pointA: {
				x: x,
				y: y,
			},
		...
		if (isAttached) {
			cons[cons.length] = Constraint.create({
				//pin last block to a point in space
				pointA: {
	...
	labs() {
		...
		exitOptions = [
			(x = offset.x, y = offset.y) => {
				...
				let isInRoom = false
				doCustom.push(() => {
					if (!isInRoom && m.pos.x > x - 100 && m.pos.x < x + 2700 && m.pos.y > y - 1300 && m.pos.y < y) {
						//check if player is in this room and run code once
						isInRoom = true
		...
		emptyOptions = [
			//nothing good here except the starting power up, and duplicated bosses
			(x = offset.x, y = offset.y) => {
				//pulse
				if (!isDoorLeft && isDoorRight) {
					//flipped, entering from the right
		...
		lootOptions = [
			//has some power up reward //field, ammo, research, gun
			(x = offset.x, y = offset.y) => {
				...
				doCustom.push(() => {
					...
					//     isInRoom = true
					// } else
					if (powerUp1.velocity.y !== 0) {
						//don't run this code if power up is gone //hack:  powerUp1.velocity.y !== 0 seems to only be true if the power up up doesn't exist and is no longer being affected by gravity
						ctx.strokeStyle = '#f0f'
						ctx.lineWidth = 2
						if (Vector.magnitudeSquared(Vector.sub(m.pos, powerUp1.position)) < 90000) {
							//zone radius is 300
		...
		upDownOptions = [
			//extra tall vertical section 3000x3000  //this is where the level boss is
			// (x = offset.x, y = offset.y) => {
			...
			(x = offset.x, y = offset.y) => {
				...
				doCustomTopLayer.push(() => {
					...
					if (toggle.isOn && !toggle.isAddedElements) {
						//this code runs once after the toggle is triggered
		...
		while (
			//makes sure that the exit and entrance aren't both on the same floor
		...
		outline = (isLower = true) => {
			...
			if (isDoorRight) {
				//if door only add wall on right side
	...
	null() {
		...
		level.customTopLayer = () => {
			...
			for (let i = -19.5; i < 20; i++) {
				...
				ctx.bezierCurveTo(sway.x * Math.abs(i) + where.x, where.y + 25 * Math.abs(i) + 60 + sway.y * Math.sqrt(Math.abs(i)), sway.x * Math.abs(i) + where.x + horizontalStep * i, where.y + 25 * Math.abs(i) + 60 + sway.y * Math.sqrt(Math.abs(i)), horizontalStep * i, -800)
			}
			ctx.lineWidth = 0.5
			ctx.strokeStyle = '#899'
			ctx.stroke()
	...
	reactor() {
		...
		level.custom = () => {
			...
			if (isDoorsLocked) {
				if (!isFightOver && !(simulation.cycle % 120)) {
					//once a second
				...
				if (player.position.x < 0) {
					//if player gets trapped inside starting room open up again
					isDoorsLocked = false
					doorIn.isClosing = false
				}
	...
	intro() {
		if (level.levelsCleared === 0) {
			//if this is the 1st level of the game
	...
	testChamber() {
		...
		level.custom = () => {
			if (!(m.cycle % 60)) {
				//so much work to catch blocks caught at the bottom of the vertical portals
		...
		const powerUpPos = shuffle([
			{
				//no debris on this level but 2 random spawn instead
				x: -150,
				y: -1775,
			},
			{
	...
	stronghold() {
		// player made level  by    Francois 👑 from discord
	...
	detours() {
		...
		if (simulation.difficulty > 2) {
			...
			if (me) {
				me.onDeath = function () {
					//please don't edit the onDeath function this causes serious bugs
					spawnCouloirEnHaut()
					doorSortieSalle.isClosing = false
				}
			} else {
	...
	house() {
		...
		var person = Body.create({
			parts: [chest, head, leftLowerArm, leftUpperArm, rightLowerArm, rightUpperArm, leftLowerLeg, rightLowerLeg, leftUpperLeg, rightUpperLeg],
		})
	...
	vats() {
		// Made by Dablux#6610 on Discord
		...
		level.custom = () => {
			...
			if (m.pos.y > 1600 && 700 < m.pos.x && m.pos.x < 3200) {
				// Saving player from slime pit
				Matter.Body.setVelocity(player, {
					x: 0,
					y: 0,
				})
	...
	'n-gon'() {
		...
		function hangingNGON(x, y, s, height, stiffness, pin, properties) {
			...
			//n
			compound3Parts(x + 100 * s, y + 310 * s, '0 0 ' + 200 * s + ' 0 ' + 200 * s + ' ' + 620 * s + ' 0 ' + 620 * s, x + 300 * s, y + 160 * s, 200 * s + ' ' + 40 * s + ' ' + 400 * s + ' ' + 40 * s + ' ' + 400 * s + ' ' + 280 * s + ' ' + 200 * s + ' ' + 280 * s, x + 499 * s, y + 333.3 * s, 400 * s + ' ' + 40 * s + ' ' + 540 * s + ' ' + 40 * s + ' ' + 600 * s + ' ' + 100 * s + ' ' + 600 * s + ' ' + 620 * s + ' ' + 400 * s + ' ' + 620 * s + ' ' + 400 * s + ' ' + 280 * s, properties)
			...
			//g
			compound3Parts(x + 1400 * s, y + 300 * s, '0 0 ' + 250 * s + ' 0 ' + 425 * s + ' ' + 175 * s + ' ' + 425 * s + ' ' + 450 * s + ' ' + 275 * s + ' ' + 600 * s + ' 0 ' + 600 * s + ' ' + -175 * s + ' ' + 425 * s + ' ' + -175 * s + ' ' + 175 * s, x + 1636 * s, y + 633 * s, 425 * s + ' ' + 450 * s + ' ' + 425 * s + ' ' + 750 * s + ' ' + 375 * s + ' ' + 800 * s + ' ' + 275 * s + ' ' + 675 * s + ' ' + 275 * s + ' ' + 600 * s, x + 1398 * s, y + 737 * s, 375 * s + ' ' + 800 * s + ' ' + -75 * s + ' ' + 800 * s + ' ' + -75 * s + ' ' + 675 * s + ' ' + 275 * s + ' ' + 675 * s, properties)
			addConstraint(x + 1500 * s, y - height, 0, -15 * s, stiffness, composite[composite.length - 1], pin)

			//o
			spawn.bodyVertex(x + 2300 * s, y + 300 * s, '0 0 ' + 250 * s + ' 0 ' + 425 * s + ' ' + 175 * s + ' ' + 425 * s + ' ' + 425 * s + ' ' + 250 * s + ' ' + 600 * s + ' 0 ' + 600 * s + ' ' + -175 * s + ' ' + 425 * s + ' ' + -175 * s + ' ' + 175 * s, properties)
			addConstraint(x + 2300 * s, y - height, 0, -10 * s, stiffness, body[body.length - 1], pin)

			//n
			compound3Parts(x + 2900 * s, y + 310 * s, '0 0 ' + 200 * s + ' 0 ' + 200 * s + ' ' + 620 * s + ' 0 ' + 620 * s, x + 3100 * s, y + 160 * s, 200 * s + ' ' + 40 * s + ' ' + 400 * s + ' ' + 40 * s + ' ' + 400 * s + ' ' + 280 * s + ' ' + 200 * s + ' ' + 280 * s, x + 3300 * s, y + 333.3 * s, 400 * s + ' ' + 40 * s + ' ' + 540 * s + ' ' + 40 * s + ' ' + 600 * s + ' ' + 100 * s + ' ' + 600 * s + ' ' + 620 * s + ' ' + 400 * s + ' ' + 620 * s + ' ' + 400 * s + ' ' + 280 * s, properties)
	...
	run() {
		...
		let runMobList = ['hopper', 'slasher', 'striker', 'stabber', 'springer', 'pulsar', 'sneaker', 'spinner', 'grower', 'focuser', 'spawner']
	...
	crouch() {
		//learn to crouch
		if (localSettings.isTrainingNotAttempted) {
			//after making it to the second training level
	...
	hold() {
		//put block on button to open door
		m.addHealth(Infinity)
		level.setPosToSpawn(60, -50) //normal spawn
		spawn.mapRect(10, -10, 100, 20) //small platform for player
		level.exit.x = 1775
	...
	throw() {
		//throw a block on button to open door
		m.addHealth(Infinity)
		level.setPosToSpawn(60, -50) //normal spawn
		spawn.mapRect(10, -10, 100, 20) //small platform for player
		level.exit.x = 1775
	...
	throwAt() {
		//throw a block at mob to open door
		m.addHealth(Infinity)
		level.setPosToSpawn(60, -50) //normal spawn
		spawn.mapRect(10, -10, 100, 20) //small platform for player
		level.exit.x = 1775
	...
	matterWave() {
		//fire matter wave through the map to kill mosb
		level.difficultyIncrease(1) //difficulty on training mode resets to zero with each new level
		level.setPosToSpawn(60, -50) //normal spawn
		spawn.mapRect(10, -10, 100, 20) //small platform for player
		level.exit.x = 1775
	...
	missile() {
		//fire a missile to kill mobs and trigger button
		level.difficultyIncrease(1) //difficulty on training mode resets to zero with each new level
		level.setPosToSpawn(60, -50) //normal spawn
		spawn.mapRect(10, -10, 100, 20) //small platform for player
		level.exit.x = 1775
	...
	mine() {
		//kill mobs and tack their bodies
		level.difficultyIncrease(1) //difficulty on training mode resets to zero with each new level
	...
	grenades() {
		//jump at the top of the elevator's path to go extra high
		level.difficultyIncrease(1) //difficulty on training mode resets to zero with each new level
		level.setPosToSpawn(0, -50) //normal spawn
