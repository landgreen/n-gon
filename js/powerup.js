...
const powerUps = {
	...
	doDuplicates() {
		//draw power ups but give duplicates some electricity
		ctx.globalAlpha = 0.4 * Math.sin(m.cycle * 0.15) + 0.6
		for (let i = 0, len = powerUp.length; i < len; ++i) {
			ctx.beginPath()
			ctx.arc(powerUp[i].position.x, powerUp[i].position.y, powerUp[i].size, 0, 2 * Math.PI)
	...
	doDuplicatesVanish() {
		...
		for (let i = 0, len = powerUp.length; i < len; ++i) {
			if (powerUp[i].isDuplicated && Math.random() < 0.004) {
				//  (1-0.004)^150 = chance to be removed after 3 seconds
	...
	endDraft(type, isCanceled = false) {
		if (isCanceled) {
			...
			if (tech.isBanish && type === 'tech') {
				// banish researched tech by adding them to the list of banished tech
	...
	ammo: {
		...
		effect() {
			if (b.inventory.length > 0) {
				if (tech.isAmmoForGun && b.activeGun) {
	...
	field: {
		...
		pick(who, skip1 = -1, skip2 = -1, skip3 = -1, skip4 = -1) {
			...
			if (powerUps.field.choiceLog.length > totalChoices || powerUps.field.choiceLog.length === totalChoices) {
				//make sure this isn't the first time getting a power up and there are previous choices to remove
				for (let i = 0; i < totalChoices; i++) {
					//repeat for each choice from the last selection
		...
		effect() {
			if (m.alive) {
				function pick(skip1 = null, skip2 = null, skip3 = null, skip4 = null) {
					...
					const totalChoices = tech.isDeterminism ? 1 : 3 + tech.isExtraChoice * 2
					if (powerUps.tech.choiceLog.length > totalChoices || powerUps.tech.choiceLog.length === totalChoices) {
						//make sure this isn't the first time getting a power up and there are previous choices to remove
						for (let i = 0; i < totalChoices; i++) {
							//repeat for each choice from the last selection
							if (options.length > totalChoices) {
					...
					if (options.length > 0) {
						...
						//if no tech options available eject banish tech
						for (let i = 0, len = tech.tech.length; i < len; i++) {
	...
	gun: {
		...
		pick(who, skip1 = -1, skip2 = -1, skip3 = -1, skip4 = -1) {
			...
			//remove repeats from last selection
			const totalChoices = tech.isDeterminism ? 1 : 3 + tech.isExtraChoice * 2
			if (powerUps.gun.choiceLog.length > totalChoices || powerUps.gun.choiceLog.length === totalChoices) {
				//make sure this isn't the first time getting a power up and there are previous choices to remove
				for (let i = 0; i < totalChoices; i++) {
					//repeat for each choice from the last selection
					if (options.length > totalChoices) {
		...
		effect() {
			...
			if (choice1 > -1) {
				...
				if (tech.isExtraChoice) {
					...
					if (choice5 > -1)
						text += `<div class="choose-grid-module" onclick="powerUps.choose('gun',${choice5})">
	...
	spawnRandomPowerUp(x, y) {
		...
		if ((Math.random() * Math.random() - 0.3 > Math.sqrt(m.health) && !tech.isEnergyHealth) || Math.random() < 0.04) {
		...
		if (Math.random() < 0.005 * (10 - level.levelsCleared)) {
			//a new tech has a low chance that decreases in later levels
	...
	addResearchToLevel() {
		//add a random power up to a location that has a mob,  mostly used to give each level one randomly placed research
		if (mob.length && Math.random() < 0.8) {
			// 80% chance
	...
	spawnStartingPowerUps(x, y) {
		...
		if (level.levelsCleared < 4) {
			//runs 4 times on all difficulty levels
	...
	spawn(x, y, target, moving = true, mode = null, size = powerUps[target].size()) {
		if ((!tech.isSuperDeterminism || target !== 'research') && !(tech.isEnergyNoAmmo && target === 'ammo') && !simulation.isNoPowerUps) {
