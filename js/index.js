...
const build = {
  pauseGrid() {
    ...
    text = "";
    text += `<div class="pause-grid-module"><div class="grid-title"><div class="circle-grid field"></div> &nbsp; ${
      mech.fieldUpgrades[mech.fieldMode].name
    }</div> ${mech.fieldUpgrades[mech.fieldMode].description}</div>`;
  ...
  choosePowerUp(who, index, type) {
    if (type === "gun") {
      ...
      if (mech.fieldMode !== index) {
        document
          .getElementById("field-" + mech.fieldMode)
          .classList.remove("build-field-selected");
        mech.setField(index);
        who.classList.add("build-field-selected");
      }
    } else if (type === "mod") {
      //remove mod if you have too many
      if (b.mods[index].count < b.mods[index].maxCount) {
        if (!who.classList.contains("build-mod-selected"))
          who.classList.add("build-mod-selected");
        b.giveMod(index);
    ...
    for (let i = 0, len = b.mods.length; i < len; i++) {
      ...
      if (b.mods[i].allowed()) {
        ...
        if (modID.classList.contains("build-grid-disabled")) {
          ...
          modID.setAttribute(
            "onClick",
            `javascript: build.choosePowerUp(this,${i},'mod')`
          );
        }
      } else {
...
<<<<<<< HEAD
    ...
    document.getElementById(
      "difficulty-select-custom"
    ).value = document.getElementById("difficulty-select").value;
    document
      .getElementById("difficulty-select-custom")
      .addEventListener("input", () => {
        game.difficultyMode = Number(
          document.getElementById("difficulty-select-custom").value
        );
        localSettings.difficultyMode = Number(
          document.getElementById("difficulty-select-custom").value
        );
        document.getElementById(
          "difficulty-select"
        ).value = document.getElementById("difficulty-select-custom").value;
  ...
  startBuildRun() {
    ...
    const increase = Math.min(
      99,
      Number(document.getElementById("starting-level").value) *
        game.difficultyMode
    );
...
if (localSettings) {
  ...
  document.getElementById("track-pad-mode").checked =
    localSettings.isEasyToAimMode;

  game.difficultyMode = localSettings.difficultyMode;
  document.getElementById("difficulty-select").value =
    localSettings.difficultyMode;
