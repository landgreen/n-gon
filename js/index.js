...
const build = {
	...
	pauseGrid() {
		...
		if (!simulation.isChoosing)
			text += `<br><span style="font-size:1.5em;font-weight: 600;">PAUSED</span> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; press P to resume
  ...
  <div style="align-items: center; text-align:center; font-size: 1.00em; line-height: 190%;background-color:var(--build-bg-color);">
    ...
    <div>
      ...
      <input type="checkbox" id="no-power-ups" name="no-power-ups" style="width:17px; height:17px;">
		...
		for (let i = 0, len = tech.tech.length; i < len; i++) {
			if (!tech.tech[i].isExperimentHide && (!tech.tech[i].isJunk || localSettings.isJunkExperiment)) {
				//&& (!tech.tech[i].isNonRefundable || tech.tech[i].isExperimentalMode)) {
				if (tech.tech[i].allowed() && (!tech.tech[i].isNonRefundable || tech.tech[i].isExperimentalMode || localSettings.isJunkExperiment)) {
	...
	startExperiment() {
		...
		if (simulation.isNoPowerUps) {
			//remove tech, guns, and fields
			function removeOne() {
				//recursive remove one at a time to avoid array problems
